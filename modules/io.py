...
def _generate_oscillating_gradient(start_color, end_color, steps):
    ...
    for i in range(half_steps):
        gradient_color = [
            int(start_color[j] + (end_color[j] - start_color[j]) * (i / half_steps)) for j in range(3)
        ]
        gradient.append(gradient_color)

    for i in range(half_steps, steps):
        gradient_color = [
            int(end_color[j] + (start_color[j] - end_color[j]) * ((i - half_steps) / half_steps)) for j in range(3)
        ]
        gradient.append(gradient_color)
